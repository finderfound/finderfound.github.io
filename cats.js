...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const SPRITE_H = 64;
  const MARGIN_X = 28;
  const MARGIN_TOP = 95;   // below sticky header
  ...
  /* ---------- NATURAL WANDER (visual-safe targets) ---------- */
  function pickNewWanderTarget(cat) {
    ...
    cat.jitter = (Math.random() - 0.5) * 28;
    cat.wanderCooldown = 1.1 + Math.random() * 1.9;
  }
  ...
  /* Occasional zoomies for Midnight when not in chase */
  setInterval(() => {
    if (chaseMode || randomChase) return;
    if (Math.random() < 0.22) {
      randomChase = true;
      playMeow();
      setTimeout(() => { randomChase = false; }, 1650);
    }
  }, 5800);
