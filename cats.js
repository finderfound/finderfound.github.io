...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /* Safe margins for visual sprites (prevents clipping + respects header)
     Cats stay fully contained in viewport. Top access enabled, bottom margin tightened. */
  const SPRITE_W = 64;
  const SPRITE_H = 64;
  const MARGIN_X = 28;
  const MARGIN_TOP = 28;   // allows wandering near the top (slight header overlap OK)
  const MARGIN_BOTTOM = 95; // tighter bottom to prevent dropping offscreen
  ...
  function pickNewWanderTarget(cat) {
    ...
    const visMaxY = window.innerHeight - SPRITE_H - MARGIN_BOTTOM - 40; // ensure fully visible
  ...
  function initCats() {
    ...
    // Safer starting Y based on screen height (middle-upper)
    const safeStartY = Math.max(180, Math.min(h * 0.45, h - 220)); // keeps visual comfortably visible
